...
describe('Menu Relationship List Controller', function() {
  ...
  beforeEach(inject(function($q, $controller, $rootScope, _menuService_, $location, $httpBackend,
    _catererStationService_, _menuCatererStationsService_, $injector) {
  ...
  describe('generating the query filter', function() {
    ...
    it(
      'create a query filter that contains a menu id and the supplied end date but maintains start date as today',
      function() {
        scope.search = {
          menuId: 6
        };
  ...
  describe('the functionality that associates menus to relationships', function() {
    ...
    it('should not have any relations or stations associated to the relationship yet', function() {
      expect(scope.relationshipList).toEqual([]);
    });

    it('should associate stations with relationships when the controller inits', function() {
      scope.$digest();
    ...
    it('should associate (1) station with the first relationship', function() {
      scope.$digest();
      expect(scope.relationshipList[0].stations.length).toEqual(1);
    });

    it('should contain the station data inside the first relationship', function() {
      scope.$digest();
