...
/**
 * @ngdoc function
 * @name ts5App.controller:ReportExchangeRateCtrl
 * @description
 * # ReportExchangeRateCtrl
...
angular.module('ts5App')
  .controller('ReportExchangeRateCtrl', function($scope, globalMenuService, currencyFactory, dateUtility,
    ...
    $scope.viewName = 'Manage Retail Company Report Exchange Rate';
    $scope.search = {};
        ...
        angular.forEach(companyCurrencies, function(currency) {
          ...
          if (!isFound && dateUtility.formatDateForAPI(currency.endDate)  > dateUtility.isTodayDatePicker()) {
            companyExchangeRates.push({
              ...
              exchangeRateType: $scope.reportExchangeRateType,
    ...
    this.denormalizeCompanyExchangeRate = function(index, exchangeRate) {
      ...
      payload.exchangeRateType = $scope.reportExchangeRateType;
    ...
    this.showSaveSuccess = function() {
      ...
      $this.showToast('success', 'Report Exchange Rate', 'Exchange rate successfully saved!');
      $this.hideLoadingModal();
    };
    ...
    $scope.saveCompanyExchangeRate = function(index, exchangeRate) {
      ...
      if (exchangeRate.id) {
        $this.showLoadingModal('Updating Report Exchange Rate Data');
        currencyFactory.updateCompanyExchangeRate(payload).then($this.showSaveSuccess, $this.showSaveErrors);
      } else {
        $this.showLoadingModal('Creating Report Exchange Rate Data');
    ...
    $scope.searchCompanyReportExchangeRates = function() {
      ...
      $scope.errorResponse = {};
      var searchPayload = $this.formatPayloadForSearch();
      if ($this.reportExchangeRateType) {
        searchPayload.exchangeRateType = $this.reportExchangeRateType.id;
        $scope.reportExchangeRateType = $this.reportExchangeRateType.id;
      }
    ...
    $scope.addNewExchangeRate = function() {
      ...
      newExchangeRate.exchangeRateType = $this.reportExchangeRateType;
      newExchangeRate.startDate = dateUtility.tomorrowFormattedDatePicker();
      newExchangeRate.endDate = dateUtility.tomorrowFormattedDatePicker();
    ...
    this.setPortalExchangeRate = function (dataFromAPI) {
      ...
      $this.reportExchangeRateType = lodash.findWhere(exchangeRateTypes, { name: 'Report Exchange Rate' });
    };
