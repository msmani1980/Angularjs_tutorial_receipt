 ...
 * @description # ScheduleReportCtrl Controller of the ts5App
 */
angular
		.module('ts5App')
		.controller(
				'ScheduleReportCtrl',
				function($scope, $modal, templateService, templateId,
						scheduleReportService, $modalInstance, $filter,
						scheduledReportsService, scheduledReportId) {
  ...
  if (scheduledReportId !== '') {
    ...
    .getSchedule($scope.scheduledReportId)
    .then(
										function(res) {
  $scope.scheduleReport = res.data;
  var timeArr = $scope.scheduleReport.recurrenceTime
  .split(':');
  ...
  $scope.dateRanges = ['Previous Day', 'Previous 7 days',
  'Previous Month'];
  ...
  var convertOptionValue = function(value, type, multiValue) {
    if (multiValue && Array.isArray(value)) {
      ...
      for (var i = 0; i < value.length; i++) {
        retValues.push(convertOptionValue(value[i],
        type, false));
      }
  ...
  var getSelectedOptions = function() {
    ...
    for (i = 0; i < $scope.template.options.length; i++) {
      if ($scope.selection.options[$scope.template.options[i].code]) {
        returnDetails.options[$scope.template.options[i].code] = convertOptionValue(
        $scope.selection.options[$scope.template.options[i].code],
  ...
  function getSelectedRecurrencePattern(payLoad) {
    ...
    if ($scope.scheduleReport.recurrencePattern === $scope.recurrencePattern[0].name) {
      payLoad.daysOfWeek = ($scope.user.daysOfWeek.length) ? $scope.user.daysOfWeek
      .join()
      : selection;
  ...
  $scope.saveReport = function() {
    ...
    payLoad.recurrenceStartDate = $filter('date')(
    startDatevalue, 'yyyy-MM-dd');
    payLoad.recurrenceEndDate = $filter('date')(
    endDatevalue, 'yyyy-MM-dd');

    payLoad.recurrenceTime = $filter('date')(
    $scope.scheduleReport.recurrenceTime, 'HH:mm');
